    ...
    if ($result = $conn->query($query)) {
      ...
      $index = 1;
        ...
        while ($row = $result->fetch_assoc()) {
            printf ('<tr> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td><a href=a href="../payment.php?price=%s">BUY</a> </td>  
              </tr>', $index, $row["HP_NAME"], $row["HP_PRICE"], $row["HP_TYPE"], $row["MODEL"], $row["HP_PRICE"]);
